...
int main(int argc, char *argv[])
{
    ...
    geometry_msgs::PointStamped camera_point;
     //创建订阅者对象
    ros::Subscriber sub=nh3.subscribe("/darknet_ros/bounding_boxes3",10,domsg);
    ros::Subscriber image_sub=nh3.subscribe<sensor_msgs::Image>("robot3/camera_link/depth/image_raw",1,depthcallback);      
    //创建发布者对象 用于发布给QT
    ros::Publisher pub = nh3.advertise<depth_take::Object>("/robot3/yolo_object",1000);
    depth_take::Object obj3;

    ros::Rate r(2);
    while (ros::ok)
    {
        //得到相机坐标系相对于odom的坐标偏移量
        transformStamped=tfbuffer.lookupTransform("robot3/odom","robot3/camera_link",ros::Time(0),ros::Duration(10.0));
        //相机坐标系和基座坐标系的转换信息设置
        camera_point.header.frame_id="robot3/camera_link";
        ...
        for(int i=0;i<length;i++){
            ...
            if(depth[i]<10.0){
                try
                {
                    ...
                    //转换坐标点（相对于odom坐标系）
                    base_footprint_point = tfbuffer.transform(camera_point,"robot3/odom");
                    ...
                    obj3.name=class_name[i].c_str();
                    obj3.x=base_footprint_point.point.x;
                    obj3.y=base_footprint_point.point.y;
                    obj3.z=base_footprint_point.point.z;
                    obj3.probability = probability[i];
                    pub.publish(obj3);
                }
