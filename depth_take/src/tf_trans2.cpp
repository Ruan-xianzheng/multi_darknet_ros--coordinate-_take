...
int main(int argc, char *argv[])
{
    ...
    geometry_msgs::PointStamped camera_point;
     // 创建订阅者对象
    ros::Subscriber sub=nh2.subscribe("/darknet_ros/bounding_boxes2",10,domsg);
    ros::Subscriber image_sub=nh2.subscribe<sensor_msgs::Image>("robot2/camera_link/depth/image_raw",1,depthcallback);    
    //创建发布者对象 用于发布给QT
    ros::Publisher pub = nh2.advertise<depth_take::Object>("/robot2/yolo_object",1000);
    depth_take::Object obj2;
    ros::Rate r(2);
    while (ros::ok)
    {

        //得到相机坐标系相对于odom的坐标偏移量
        transformStamped=tfbuffer.lookupTransform("robot2/odom","robot2/camera_link",ros::Time(0),ros::Duration(10.0));
        //相机坐标系和基座坐标系的转换信息设置
        camera_point.header.frame_id="robot2/camera_link";
        ...
        for(int i=0;i<length;i++){
            ...
            if(depth[i]<10.0){
                try
                {
                    ...
                    //转换坐标点（相对于odom坐标系）
                    base_footprint_point = tfbuffer.transform(camera_point,"robot2/odom");
                    ...
                    obj2.name=class_name[i].c_str();
                    obj2.x=base_footprint_point.point.x;
                    obj2.y=base_footprint_point.point.y;
                    obj2.z=base_footprint_point.point.z;
                    obj2.probability = probability[i];
                    pub.publish(obj2);
                }
