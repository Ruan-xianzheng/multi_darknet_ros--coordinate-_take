...
void domsg(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg)
{
        ...
        length=msg->bounding_boxes.size(); //画面中标记框个数
...
int main(int argc, char *argv[])
{
    ...
    // 创建订阅者对象
    ros::Subscriber sub=nh1.subscribe("/darknet_ros/bounding_boxes1",1,domsg);
    ros::Subscriber image_sub=nh1.subscribe<sensor_msgs::Image>("robot1/camera_link/depth/image_raw",1,depthcallback);    
    //创建发布者对象 用于发布给QT
    ros::Publisher pub = nh1.advertise<depth_take::Object>("/robot1/yolo_object",1);
    ros::Rate r(2);
    depth_take::Object obj;
    while (ros::ok)
    {
        // ros::Duration(0.01).sleep(); 
        //得到相机坐标系相对于odom的坐标偏移量
        transformStamped=tfbuffer.lookupTransform("robot1/odom","robot1/camera_link",ros::Time(0),ros::Duration(10.0));
        //相机坐标系和基座坐标系的转换信息设置
        camera_point.header.frame_id="robot1/camera_link";
        ...
        for(int i=0;i<length;i++){
            ...
            if(depth[i]<10.0){
                try
                {
                    ...
                    //转换坐标点（相对于odom坐标系）
                    base_footprint_point = tfbuffer.transform(camera_point,"robot1/odom");
                    ROS_INFO("类别:%s,坐标:x=%.3f米,y=%.3f米,z=%.3f米,rate=%.2f%",class_name[i].c_str(),base_footprint_point.point.x,base_footprint_point.point.y,0.0,probability[i]*100);
                    //组织被发布的消息，编写发布逻辑并发布消息
                    obj.name=class_name[i].c_str();
                    obj.x=base_footprint_point.point.x;
                    obj.y=base_footprint_point.point.y;
                    obj.z=base_footprint_point.point.z;
