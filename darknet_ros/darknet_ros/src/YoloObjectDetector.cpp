...
detection* YoloObjectDetector::avgPredictions(network* net, int* nboxes) {
  ...
  // 输入参数：network、图像的原始w和h，thresh是指过滤bbox的时候用到的阈值，hier这个阈值在yolov3中没有用到， map=0和relative=1用途不太明显看后面分析吧，num为检测到的bbox数目(具体是哪个阶段的：to add)
...
//publishInThread函数用于广播发布目标检测的结果 包括物体类别 预测框信息
void* YoloObjectDetector::publishInThread() {
  ...
  int num = roiBoxes_[0].num;  //num是roiBoxes_中识别框的数量
  if (num > 0 && num <= 100) {
    for (int i = 0; i < num; i++) {
      for (int j = 0; j < numClasses_; j++) {  //numClasses_是j类标记框的编号
        if (roiBoxes_[i].Class == j) {
          rosBoxes_[j].push_back(roiBoxes_[i]);
          rosBoxCounter_[j]++;    //rosBoxCounter_[j]是j类标记框的数量
        }
      }
    }
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) { //如果i类标记框的个数少于0，则表示没有该类别的标记框
        darknet_ros_msgs::BoundingBox boundingBox;

        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          int xmin = (rosBoxes_[i][j].x - rosBoxes_[i][j].w / 2) * frameWidth_;//ROS节点中存储的是相对于图像宽度的比例值
